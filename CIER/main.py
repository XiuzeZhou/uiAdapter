...
def main(args):
    ...
    #for split_index in ['1']:
        ...
        if args.use_uiadapter:
            model = uiAdapter(user_num=user_num, 
                               item_num=item_num, 
                               ...
                               bal_reg=args.bal_reg).to(device)
        else:
        ...
        if args.only_eval == False:
            for epoch in range(0, args.epochs):
                ...
                if best_loss<valid_loss:
                    ...
                    if args.use_uiadapter:
                        moe_ckpt_path = args.ckpt_dir + args.dataset_name + '/' + split_index + 'moe_model.pth'
                        trainable_params = {k: v for k, v in model.named_parameters() if v.requires_grad}
                        torch.save(trainable_params, moe_ckpt_path)   # Only save LoRAs
                    else:
        ...
        if args.use_uiadapter:
            ...
            model.load_state_dict(torch.load(moe_ckpt_path, map_location="cpu"), strict=False) # Only LoRAs
        else:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_name', default='../autodl-fs/Mistral-7B-Instruct-v0.3/', type=str)
    parser.add_argument('--ckpt_dir', default='../autodl-tmp/models/mistral/', type=str)
    parser.add_argument('--log_dir', default='./log/', type=str)
    parser.add_argument('--log_name', default='llama.log', type=str)
    parser.add_argument('--output_dir', default='./output/', type=str)
    parser.add_argument('--use_uiadapter', action='store_true', help='Use uiLoRA MoE Architecture')
    parser.add_argument('--lora_modules', '--lora_modules', type=int, default=2, help='number of modules for LoRA')
    parser.add_argument('--r', '--r', type=int, default=8, help='rank for LoRA')
    parser.add_argument('--expert_num', '--expert_num', type=int, default=4, help='the number of experts')
    parser.add_argument('--top_k', '--top_k', type=int, default=2, help='top-k experts')
    parser.add_argument('--bal_reg', '--bal_reg', type=float, default=0.1, help='regularization on MoE load balancing task')
